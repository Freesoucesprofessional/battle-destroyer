...
import api               from '../utils/apiClient';   // ← replaces all crypto helpers
...
export default function Dashboard({ toggleTheme, theme, setIsAuth }) {
    ...
    useEffect(() => {
        const fetchUser = async () => {
            ...
            try {
                // One line — interceptor handles all crypto
    ...
    useEffect(() => {
        ...
        const ctx = gsap.context(() => {
            gsap.fromTo(bannerRef.current, { opacity: 0, y: -40 }, { opacity: 1, y: 0, duration: 0.7, ease: 'power3.out', delay: 0.1 });
            gsap.fromTo(
                statsRef.current?.querySelectorAll('.stat-card'),
                { opacity: 0, y: 50, scale: 0.9 },
                { opacity: 1, y: 0, scale: 1, duration: 0.55, ease: 'back.out(1.4)', stagger: 0.1, delay: 0.3 }
            );
            gsap.fromTo(referralRef.current, { opacity: 0, x: -50 }, { opacity: 1, x: 0, duration: 0.6, ease: 'power3.out', scrollTrigger: { trigger: referralRef.current, start: 'top 88%' } });
            gsap.fromTo(
                howRef.current?.querySelectorAll('.how-card'),
                { opacity: 0, y: 40 },
                { opacity: 1, y: 0, duration: 0.55, ease: 'power2.out', stagger: 0.12, scrollTrigger: { trigger: howRef.current, start: 'top 85%' } }
            );
            if (warningRef.current) {
                gsap.fromTo(warningRef.current, { opacity: 0, scale: 0.95 }, { opacity: 1, scale: 1, duration: 0.5, ease: 'power2.out', scrollTrigger: { trigger: warningRef.current, start: 'top 90%' } });
            }
            gsap.to('.launch-btn', { boxShadow: '0 6px 36px rgba(220,38,38,0.55)', repeat: -1, yoyo: true, duration: 1.6, ease: 'sine.inOut' });
        });
        return () => ctx.revert();
    }, [user]);
        ...
        <div className={`relative min-h-screen transition-colors duration-300 ${dark ? 'bg-surface-950' : 'bg-slate-50'}`}>
            ...
            <div className="relative z-10">
                ...
                <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
                    ...
                    {/* Welcome Banner */}
                    <div ref={bannerRef} className={`rounded-2xl p-5 sm:p-7 border mb-5 relative overflow-hidden transition-all ${cardCls}`} style={{ opacity: 0 }}>
                    ...
                    <div ref={howRef} className={`rounded-2xl p-5 sm:p-6 border transition-all ${cardCls}`}>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                            {howItWorks.map((item, i) => (
                                <div key={i} className={`how-card rounded-xl p-4 border transition-all ${dark ? 'bg-white/[0.03] border-white/[0.06] hover:bg-white/[0.05]' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'}`} style={{ opacity: 0 }}>
                    ...
                    {!user.isPro && user.credits === 0 && (
                        <div ref={warningRef} className="mt-4 rounded-2xl px-5 py-4 flex items-start gap-3 border" style={{ background: 'rgba(234,179,8,0.06)', borderColor: 'rgba(234,179,8,0.2)', opacity: 0 }}>
