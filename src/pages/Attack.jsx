...
export default function Attack({ toggleTheme, theme, setIsAuth }) {
  ...
  const launch = async () => {
    ...
    try {
      ...
      setLaunchError(msg);
      if (err.response?.data?.credits !== undefined) setUser(prev => ({ ...prev, credits: err.response.data.credits }));
      resetCaptcha();
    } finally {
      setLaunching(false);
    }
                  ...
                  {attackHistory.length > 0 && (
                    <button
                      ...
                      className={`p-2 rounded-lg text-xs transition-all ${
                        dark ? 'bg-red-600/10 hover:bg-red-600/20 text-red-400' : 'bg-red-50 hover:bg-red-100 text-red-500'
                      }`}
                      title="Clear history"
                    >
                      <FaTrash size={11} />
